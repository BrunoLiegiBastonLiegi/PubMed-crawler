...
fil = 'cluster-test.txt'
#fil = 'covid-separated-nodes.txt'
with open(fil) as r:
    lines = r.readlines()
lines = [l.replace('\n','') for l in lines ]
del_list = []
#for v in lines:
    #del_list.append(f.get_vertex(v.replace('\n','')))
'''
for v in f.get_vertices():
    if f.get_vertex(v) not in lines:
        del_list.append(v)
        print(False)
f.remove_vertices(del_list)
'''
'''
g = MultilayerGraph(['CAUSES'])

for e in f.get_edges():
    try:
        g.add_edge(e[2],f.get_vertex(e[0]),f.get_vertex(e[1]), w=e[3])
    except:
        g.add_layer(e[2])
        g.add_edge(e[2],f.get_vertex(e[0]),f.get_vertex(e[1]), w=e[3])
'''
